...
class ApproxMPCSettings:
    ...
    # Input variables
    input_vars: Tuple = ("x1_0","x2_0","u_prev_0")
    output_vars: Tuple = ("u0","u1")
...
class ApproxMPC():
    ...
    def set_scaling_from_bounds(self):
        x_min = torch.tensor([self.settings.lb_x1,self.settings.lb_x2,self.settings.lb_u1,self.settings.lb_u2])
        x_max = torch.tensor([self.settings.ub_x1,self.settings.ub_x2,self.settings.ub_u1,self.settings.ub_u2])
        x_range = x_max-x_min
        x_shift = x_min

        y_min = torch.tensor([self.settings.lb_u1,self.settings.lb_u2])
        y_max = torch.tensor([self.settings.ub_u1,self.settings.ub_u2])
        y_range = y_max-y_min
        y_shift = y_min
