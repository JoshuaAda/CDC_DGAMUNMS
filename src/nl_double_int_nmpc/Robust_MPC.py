...
class RobustMPC:
    def __init__(self):
        ...
        p_max = np.array([0.3])
        model._rhs['x']
        system = Function('system', [model.x, model.u, model.p], [model._rhs['x']])
        ...
        lb_opt_x['u'] = lb_u
        ub_opt_x['u'] = ub_u
        self.lb_opt_x=lb_opt_x
        self.ub_opt_x=ub_opt_x
        # %%
        ...
        self.jacobian = Function('jacobian', [opt_x], [jacobian(self.g, opt_x)])
        self.prob = {'f': J, 'x': opt_x.cat, 'g': self.g,
    ...
    def make_step(self, x0):
        ...
        nx=2
        x_ref=np.array([[5],[2]])
        lb_x = 0 * np.ones((1600, 1))
        ub_x = 10 * np.ones((1600, 1))
        # input constraints
        lb_u = np.array([[-10], [-5]])  # -10*np.ones((nu,1))
        ub_u = np.array([[10], [5]])  # 10*np.ones(

        sol = self.rob_mpc_solver(x0=self.opt_x, lbx=self.lb_opt_x, ubx=self.ub_opt_x, lbg=self.lb_g, ubg=self.ub_g,
                             p=vertcat(x0,x0, x_ref, self.u0, 0.3, 0.0,np.array([[10], [6.43288]]),np.array([[-4.49345e-09], [-4.62047e-09]])))
        u0=self.opt_x(sol['x'])['u'][0][0].full()
        print(sol['f'].full())
        self.opt_x=self.opt_x(sol['x'])
        self.u0=u0
        return u0